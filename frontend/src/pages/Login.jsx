  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.access_token) {
        loginUser({ username }, data.access_token);
        navigate('/dashboard');
      }
    } catch (err) {
    ...
    <div className="animate-fade-in" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '70vh', padding: '2rem' }}>
      <div className="glass-panel" style={{ width: '100%', maxWidth: '420px', padding: '2.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p className="text-secondary" style={{ fontSize: '0.9rem' }}>Sign in to your SmartHire AI account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <button type="submit" className="btn btn-primary" style={{ width: '100%', height: '44px' }} disabled={loading}>
            {loading ? <div className="spinner" style={{ width: '16px', height: '16px' }}></div> : 'Sign In'}
          </button>
        </form>

        <p style={{ textAlign: 'center', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
          Don't have an account? <Link to="/signup" style={{ color: 'var(--accent-light)', textDecoration: 'none', fontWeight: 600 }}>Get Started</Link>
        </p>
      </div>
    </div>
  );
